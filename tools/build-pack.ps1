...
本包包含当前 D2R 字符串汉化资源、legacy 字符串补丁、D2RLAN 过滤器汉化覆盖和带 ACT 前缀的恐怖区域提示名称汉化，不包含完整模组、存档、高清贴图、完整 SunRise Filter 或 Launcher/HUD DLL。右上角倒计时 `Next Rotation In` 来自 Launcher/HUD DLL 的硬编码文本，本包不翻译也不修改 DLL，以避免启动器或游戏崩溃。符文名按当前项目规则保留英文。
